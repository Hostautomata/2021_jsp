			...
			<tr>
				<td colspan="4">
					...
					<c:choose>
						...
						<c:otherwise>
							<span style="color: tomato; padding: 5px; border: 1px solid tomato"><a href="${pageContext.request.contextPath}/MyController?cmd=list&cPage=${pvo.beginBlock-pvo.pagePerBlock}">이전으로</a></span>
						</c:otherwise>
					</c:choose>
					&nbsp;&nbsp;
					<%-- 번호 --%>
				   <c:forEach begin="${pvo.beginBlock }" end="${pvo.endBlock }" step="1" var="k">
					...
					<c:choose>
						...
						<c:otherwise>
							<span style="color: tomato; padding: 5px; border: 1px solid tomato"><a href="${pageContext.request.contextPath}/MyController?cmd=list&cPage=${pvo.beginBlock+pvo.pagePerBlock}">다음으로</a></span>
						</c:otherwise>
					</c:choose>
				</td>
